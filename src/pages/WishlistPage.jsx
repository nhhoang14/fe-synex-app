      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <article className="rounded-3xl border border-border bg-slate-50 p-5 shadow-sm md:col-span-2">
          ...
          <div className="mt-2 flex flex-wrap gap-3">
            <Link
              to={ROUTES.PRODUCTS}
              className="rounded-full border border-border bg-white px-4 py-2 font-semibold text-ink transition hover:bg-slate-50"
            >
              Xem sản phẩm
            </Link>
            <Link
      ...
      <section className="rounded-[28px] border border-border bg-white p-6 shadow-sm">
        ...
        {products.length === 0 ? (
          <p className="mt-4 text-slate-600">
            Bạn chưa có sản phẩm nào trong danh sách đã thích.
          </p>
        ) : (
